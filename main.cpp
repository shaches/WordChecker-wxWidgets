    ...
    : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(900, 600))
{
    ...
    m_richTextCtrlRight = new wxRichTextCtrl(mainPanel, wxID_ANY, wxEmptyString,
                                             wxDefaultPosition, wxDefaultSize,
                                             wxVSCROLL | wxHSCROLL | wxBORDER_SUNKEN | wxTE_READONLY);
   ...
   void MyFrame::OnButton1Click(wxCommandEvent& event)
   {
		...
		std::vector<std::array<std::string,2>> index;
		...
		for(int i = 0; i < index.size()-1; i++){
			for(int j = 0; j < index.size()-1-i; j++){
				if(index.at(j).at(0) > index.at(j+1).at(0)){
		...
		int count{0};
		for(int i = 0; i < index.size()-1; i++){
			if(index.at(i).at(0) == index.at(i+1).at(0)){
				index.erase(index.begin() + i);
				i -= 1;
				count++;
			}
			else{
				index.at(i).at(1) = std::to_string(count+1);
				count = 0;
			}

			if(i == index.size()-2){
				index.at(i+1).at(1) = std::to_string(count+1);
			}
		}

		for(int i = 0; i < index.size()-1; i++){
			for(int j = 0; j < index.size()-1-i; j++){
				if(std::stoi(index.at(j).at(1)) < std::stoi(index.at(j+1).at(1))){
					std::swap(index.at(j), index.at(j+1));
				}
			}
		}
		...
		for(int i = 0; i < index.size(); i++){
			text += index.at(i).at(0) + " : " + index.at(i).at(1) + '\n';
		}
